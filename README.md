...
* Copy and paste the contents of the `/apaxy` folder to the root level of your webserver so that the `theme` folder would be at `www.mydomain.com/theme`.
* Rename `htaccess.txt` to `.htaccess` in both the `/` and `/theme` folders.
